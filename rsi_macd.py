...
def run_strategy_rsi_macd_crossover(stock, invest_cap, turnover, min_trade_bal, is_crypto, rf_symbol):
    ...
    def calculate_technical_indicators(): 
        ...
        def macd(df, a=12, b=26, c=9):
            df = df.copy()
            df['ma_fast'] = df['Close'].ewm(span=a, min_periods=0).mean()
            df['ma_slow'] = df['Close'].ewm(span=b, min_periods=0).mean()
        ...
        macd_df = macd(temp,12,26,9)
        macd_df.columns = ['macd', 'macd_signal']

        rsi_df = rsi_values.to_frame(name='RSI')
        rsi_df.dropna(how='any',inplace=True)
        macd_df.dropna(how='any',inplace=True)

        df = pd.concat([rsi_df,macd_df],axis=1)
        df['Close'] = temp['Close']
        df['Change'] = df['Close'] - df['Close'].shift(1)
        df['Low'] = temp['Low']
        df['High'] = temp['High']
