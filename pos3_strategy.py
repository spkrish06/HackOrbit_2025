...
def run_strategy_3pos(stock, invest_cap, turnover, min_trade_bal, is_crypto, rf_symbol):
    ...
    def calculate_technical_indicators(): 
        ...
        rsi_values = []
        rsi_values = rsi(temp,14)
        rsi_ma_values = []
        rsi_ma_values = sma(rsi_values,20)

        rsi_df = rsi_values.to_frame(name='RSI')
        rsi_df.dropna(how='any',inplace=True)
        rsi_ma_df = rsi_ma_values.to_frame(name='RSI_MA')
        rsi_ma_df.dropna(how='any',inplace=True)

        df = pd.concat([rsi_df,rsi_ma_df],axis=1)
        df['Close'] = temp['Close']
        df['Change'] = df['Close'] - df['Close'].shift(1)
        df['Low'] = temp['Low']
        df['High'] = temp['High']
    ...
    def signals_generation(): 
        ...
        for i in range(2, len(df)):
            if (df['Change'].iloc[i] > 0 and
                df['Change'].iloc[i-1] > 0 and
                df['Change'].iloc[i-2] > 0):

                if df['RSI'].iloc[i] > df['RSI_MA'].iloc[i]:
