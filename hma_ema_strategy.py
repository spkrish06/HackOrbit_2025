...
def run_strategy_hma_ema(stock, invest_cap, turnover, min_trade_bal, is_crypto, rf_symbol):
    ...
    def calculate_technical_indicators(): 
        ...
        rsi_values_14 = rsi(temp,14)
        rsi_ma_values_hull = hma(rsi_values_14,20)

        rsi_values_25 = rsi(temp,25)
        rsi_ma_values_expo = ema(rsi_values_25,14)

        rsi_ma_df_hull = rsi_ma_values_hull.to_frame(name='RSI_MA_Hull_14')
        rsi_ma_df_hull.dropna(how='any',inplace=True)

        rsi_ma_df_expo = rsi_ma_values_expo.to_frame(name='RSI_MA_Expo_25')
        rsi_ma_df_expo.dropna(how='any',inplace=True)

        df = pd.concat([rsi_ma_df_hull, rsi_ma_df_expo],axis=1)
        df['Close'] = temp['Close']
        df['Change'] = df['Close'] - df['Close'].shift(1)
        df['Low'] = temp['Low']
        df['High'] = temp['High']
