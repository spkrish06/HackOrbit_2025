...
def run_strategy_adx_dmi(stock, invest_cap, turnover, min_trade_bal, is_crypto, rf_symbol):
    ...
    def calculate_technical_indicators(): 
        ...
        df['Low'] = temp['Low']
        df['Close'] = temp['Close']
        df['Volume'] = temp['Volume']
        df['Change'] = df['Close'] - df['Close'].shift(1)
        df[['ADX','+DI','-DI']] = calculate_adx_dmi(df)
        return df
